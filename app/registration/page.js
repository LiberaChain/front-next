...
import IntroStep from './components/step-0-intro';
import CreateDidStep from './components/step-1-create';
import SuccessStep from './components/step-2-success';
  ...
  const handleRegistration = async () => {
    ...
    try {
      ...
      if (blockchainVerification.verified) {
        await storeIdentityAndAuth(didIdentifier, displayName, walletAddress, keyPair);
        setStep(2);
        return;
      }
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-gray-700">
          ...
          <div className="flex items-center justify-center mb-8">
            <div className={`h-2 w-2 rounded-full ${step >= 0 ? 'bg-emerald-500' : 'bg-gray-600'}`}></div>
            <div className={`h-0.5 w-12 ${step >= 1 ? 'bg-emerald-500' : 'bg-gray-600'}`}></div>
