...
import IPFSCIDLink from '../_components/IPFSCIDLink';
import { Messages } from '../_core/libera/Messages';
...
export default function ChatPage() {
    ...
    useEffect(() => {
        const loadFriends = async () => {
            try {
                ...
                const friendshipsDid = await Friendships.getFriendsDids(profileData.did);
                console.debug("User friendships DIDs:", friendshipsDid);

                const friendsData = [];
                for (const did of friendshipsDid) {
                    const friendProfile = await Profiles.getProfile(did, true);
                    if (friendProfile && friendProfile.displayName) {
                        friendsData.push(friendProfile);
                    } else {
                        // Fallback to DID if no username found
                        friendsData.push({
                            did: did,
                            displayName: `User-${did.substring(9, 13)}`
                        });
                    }
                }

                console.log("User friends DIDs:", friendsData);
    ...
    const loadIPFSConversationMessages = async () => {
    ...
    const updateConversationMessages = async (did, otherDid) => {
        // Initialize messages for this conversation if needed
        const messages = await Messages.getMessages(did, otherDid);
        console.log("Loaded messages for conversation:", otherDid, messages);
    ...
    const handleConversationClick = async (conversation) => {
        ...
        if (conversation.did) {
            await updateConversationMessages(profileData.did, conversation.did);

            // Set up refresh interval
            setInterval(updateConversationMessages, 10000, profileData.did, conversation.did);

            // // Update last message
            // setFriendsList((prevFriends) =>
            //     prevFriends.map((friend) => {
            //         if (friend.did === conversation.did) {
            //             return {
            //                 ...friend,
            //                 lastMessage: messages[messages.length - 1]?.text || '',
            //                 timestamp: messages[messages.length - 1]?.timestamp || 'New'
            //             };
            //         }
            //         return friend;
            //     })
    ...
    const handleSendMessage = async (e) => {
        ...
        if (messageInput.trim() && selectedConversation) {
            ...
            const newMessage = {
                id: Date.now(),
                // sender: 'You',
                sender: profileData?.displayName || 'You',
                text: messageInput,
            ...
            try {
                ...
                if (messageMode === 'standard' && selectedConversation.did) {
                    // Send over IPFS and store locally
                    const result = await Messages.sendMessage(profileData.did, selectedConversation.did, messageInput);
                    if (!result) {
                        throw new Error(result.error || 'Failed to send message');
                    }

                    console.log("Message sent via standard mode:", result);
        ...
        <div className="flex h-screen animate-gradient bg-gradient-to-br from-gray-900 to-gray-800 relative">
            ...
            <div className={`${isToggled ? "w-0" : "w-full"} md:w-1/3 bg-gray-800 border-r border-gray-700 overflow-hidden`}>
                <div className="p-4 border-b border-gray-700 flex justify-between items-center gap-5">
                    <Link href="/dashboard">
                        <Image src={'/logo.svg'} width={25} height={25} alt="LiberaChain Logo" />
                    </Link>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <Link href="/friends" className="block hover:cursor-pointer">
                ...
                {displayItems.length > 0 ? (
                    displayItems.map((item) => (
                        <div
                            ...
                            className={`p-4 cursor-pointer hover:bg-gray-700 ${selectedConversation?.did === item.did || selectedConversation?.id === item.id ? 'bg-gray-700' : ''
                                }`}
                        >
                            <div className="flex justify-between">
                                <h2 className="font-medium text-white">{item.name || item.displayName}</h2>
                                {item.did && (
                                    <span className="text-sm text-gray-400">{item.did.substring(0, 15)}...{item.did.substring(item.did.length - 4)}</span>
                                )}
                ...
                {selectedConversation ? (
                    <>
                        ...
                        <div className="p-4 bg-gray-800 border-b border-gray-700 flex justify-between">
                            <h2 className="text-lg font-semibold text-white">{selectedConversation.name || selectedConversation.displayName}</h2>
                                ...
                                messages[selectedConversation.did] && messages[selectedConversation.did].length > 0 ? (
                                    messages[selectedConversation.did].map((message) => (
                                        <div
                                            ...
                                            className={`mb-4 flex ${message.sender === 'You' || message.from === profileData?.did || message.sender === profileData?.displayName ? 'justify-end' : 'justify-start'
                                                }`}
                                        >
                                            <div
                                                className={`max-w-xs p-3 rounded-lg ${message.sender === 'You'
                                    ...
                                    <div className="flex items-center justify-center h-full">
                                        <div className="text-center">
                                            ...
                                            <p className="text-sm text-gray-300 mt-1">Send a message to start chatting</p>
                                        </div>
                                    </div>
                                )
                            ) : (
                    ...
                    <>
                        ...
                        {/* Messages */}
                        <div className="flex-1 p-4 overflow-y-auto bg-gray-800/50 justify-center items-center flex">
