          ...
          <div className="space-y-4">
            {sentRequests.length === 0 ? (
              ...
              sentRequests.map((request) => (
                <div key={request.id} className="flex items-center justify-between bg-gray-700/50 p-4 rounded-lg">
                  <div>
                    <p className="text-white font-medium">{request.to}</p>
                    <p className="text-sm text-gray-400">Sent {new Date(request.timestamp).toLocaleDateString()}</p>
                  </div>
              ...
              pendingRequests.map((request) => (
                <div key={request.id} className="flex items-center justify-between bg-gray-700/50 p-4 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">Received {new Date(request.timestamp).toLocaleDateString()}</p>
                  </div>
