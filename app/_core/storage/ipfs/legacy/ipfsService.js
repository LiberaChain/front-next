...
export const hasIpfsCredentials = () => {
  try {
    return (
      process.env.NEXT_PUBLIC_IPFS_HOST &&
...
const getLocalStorageItemCount = () => {
  try {
    const profiles = JSON.parse(
      localStorage.getItem("liberaChainIpfsProfiles") || "{}"
    );
...
export const uploadProfileToIPFS = async (profileData) => {
  try {
    ...
    const { cid, ipnsName } = await updateContent(
      profileData.did,
      profileData,
      "profile"
    );
  ...
  try {
    ...
    if (!hasIpfsCredentials()) {
      ...
      const dataMap = JSON.parse(
        localStorage.getItem("liberaChainIpfsData") || "{}"
      );
      const mockCid = "mock-cid-" + Date.now();
